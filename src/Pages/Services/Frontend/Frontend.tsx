...
function Frontend() {
    ...
    return Object.entries(sqlData).map(([level, questions], levelIndex) => (
      <div key={levelIndex}>
        <h3 className="text-xl md:text-2xl font-semibold mb-4 text-gray-900 dark:text-white">
          {level} Frontend Interview Questions
        </h3>
  ...
  const handleButtonClick = () => {
    if (decodedToken) {
      navigate(`${location.pathname}/react`);
    } else {
      navigate("/signin");
    }
  };
      ...
      <div id="content-to-download">
        ...
        <div className="mt-12 flex flex-col md:flex-row md:justify-between">
          <div className="md:flex-grow">
            <h3 className="text-xl md:text-2xl font-semibold mb-4 text-gray-900 dark:text-white">
              Frontend Interview Preparation Tips
            </h3>
            <p className="text-sm md:text-base lg:text-lg mb-4 text-gray-600 dark:text-gray-300">
            Be proficient in core concepts like JSX, components (functional/class), props, state, event handling, and the component lifecycle. Master React Hooks (useState, useEffect, useContext, etc.) and understand their role in functional components.
            </p>
